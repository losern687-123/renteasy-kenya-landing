export const StatsStrip = () => {
  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-3 py-14 md:py-16 bg-card border-y border-primary/10">
      {stats.map((s, i) => (
        <div
          key={s.label}
          className={`flex flex-col items-center px-6 py-6 md:py-0 ${
            i < stats.length - 1 ? "md:border-r border-primary/10" : ""
          }`}
        >
          <span className="text-4xl md:text-5xl font-serif font-light text-primary mb-2">
            {s.value}
          </span>
